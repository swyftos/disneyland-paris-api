...
public final class UnicastSubject<T> extends Subject<T> {
    ...
    void drainNormal(Observer observer) {
    ...
    void drainFused(Observer observer) {
        SpscLinkedArrayQueue spscLinkedArrayQueue = this.queue;
        boolean z = this.delayError;
        int iAddAndGet = 1;
        while (!this.disposed) {
    ...
    void errorOrComplete(Observer observer) {
        this.downstream.lazySet(null);
    ...
    boolean failedFast(SimpleQueue simpleQueue, Observer observer) {
        Throwable th = this.error;
        if (th == null) {
            return false;
        }
