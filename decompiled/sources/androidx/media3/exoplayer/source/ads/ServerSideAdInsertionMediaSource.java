...
public final class ServerSideAdInsertionMediaSource extends BaseMediaSource implements MediaSource.MediaSourceCaller, MediaSourceEventListener, DrmSessionEventListener {
    ...
    private ImmutableMap adPlaybackStates = ImmutableMap.of();
    ...
    public /* synthetic */ void lambda$setAdPlaybackStates$0(ImmutableMap immutableMap, Timeline timeline) {
        ...
        for (V v : this.mediaPeriods.values()) {
            AdPlaybackState adPlaybackState2 = (AdPlaybackState) immutableMap.get(v.periodUid);
            if (adPlaybackState2 != null) {
                v.updateAdPlaybackState(adPlaybackState2);
            }
        }
    ...
    public MediaPeriod createPeriod(MediaSource.MediaPeriodId mediaPeriodId, Allocator allocator, long j) {
        ...
        if (sharedMediaPeriod == null && ((sharedMediaPeriod = (SharedMediaPeriod) Iterables.getLast(this.mediaPeriods.get((ListMultimap) pair), null)) == null || !sharedMediaPeriod.canReuseMediaPeriod(mediaPeriodId, j))) {
    ...
    private MediaPeriodImpl getMediaPeriodForEvent(MediaSource.MediaPeriodId mediaPeriodId, MediaLoadData mediaLoadData, boolean z) {
        ...
        List list = this.mediaPeriods.get((ListMultimap) new Pair(Long.valueOf(mediaPeriodId.windowSequenceNumber), mediaPeriodId.periodUid));
        if (list.isEmpty()) {
            return null;
        }
        if (z) {
    ...
    private static final class SharedMediaPeriod implements MediaPeriod.Callback {
        ...
        public void onPrepared(MediaPeriod mediaPeriod) {
            ...
            for (int i = 0; i < this.mediaPeriods.size(); i++) {
                ((MediaPeriodImpl) this.mediaPeriods.get(i)).onPrepared();
            }
        }
