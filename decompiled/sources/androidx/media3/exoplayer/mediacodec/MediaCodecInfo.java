    ...
    public boolean isVideoSizeAndRateSupportedV21(int i, int i2, double d) {
        ...
        if (Util.SDK_INT >= 29) {
            ...
            if (iAreResolutionAndFrameRateCovered == 1) {
                logNoSupport("sizeAndRate.cover, " + i + "x" + i2 + GherkinLanguageConstants.TAG_PREFIX + d);
                return false;
            }
        }
        if (!areSizeAndRateSupportedV21(videoCapabilities, i, i2, d)) {
            if (i >= i2 || !needsRotatedVerticalResolutionWorkaround(this.name) || !areSizeAndRateSupportedV21(videoCapabilities, i2, i, d)) {
                logNoSupport("sizeAndRate.support, " + i + "x" + i2 + GherkinLanguageConstants.TAG_PREFIX + d);
                return false;
            }
            logAssumedSupport("sizeAndRate.rotated, " + i + "x" + i2 + GherkinLanguageConstants.TAG_PREFIX + d);
        }
        return true;
    }
