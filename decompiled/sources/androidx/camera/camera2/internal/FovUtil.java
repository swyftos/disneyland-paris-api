...
public class FovUtil {
    ...
    public static int focalLengthToViewAngleDegrees(float f, float f2) {
        Preconditions.checkArgument(f > BitmapDescriptorFactory.HUE_RED, "Focal length should be positive.");
        Preconditions.checkArgument(f2 > BitmapDescriptorFactory.HUE_RED, "Sensor length should be positive.");
        int degrees = (int) Math.toDegrees(Math.atan(f2 / (f * 2.0f)) * 2.0d);
        Preconditions.checkArgumentInRange(degrees, 0, Currencies.IDR, "The provided focal length and sensor length result in an invalid view angle degrees.");
        return degrees;
    }
