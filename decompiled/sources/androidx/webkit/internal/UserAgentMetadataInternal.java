...
public class UserAgentMetadataInternal {
    static Map convertUserAgentMetadataToMap(UserAgentMetadata userAgentMetadata) {
    ...
    private static String[][] getBrandVersionArray(List list) {
        if (list == null || list.isEmpty()) {
            return null;
        }
        String[][] strArr = (String[][]) Array.newInstance((Class<?>) String.class, list.size(), 3);
        for (int i = 0; i < list.size(); i++) {
            strArr[i][0] = ((UserAgentMetadata.BrandVersion) list.get(i)).getBrand();
            strArr[i][1] = ((UserAgentMetadata.BrandVersion) list.get(i)).getMajorVersion();
            strArr[i][2] = ((UserAgentMetadata.BrandVersion) list.get(i)).getFullVersion();
        }
        return strArr;
    }

    static UserAgentMetadata getUserAgentMetadataFromMap(Map map) {
