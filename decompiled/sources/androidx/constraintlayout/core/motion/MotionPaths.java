...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(MotionKeyPosition motionKeyPosition, MotionPaths motionPaths, MotionPaths motionPaths2) {
        ...
        float f16 = zIsNaN ? 0.0f : motionKeyPosition.mAltPercentY;
        if (!Float.isNaN(motionKeyPosition.mPercentY)) {
            f = motionKeyPosition.mPercentY;
        }
        if (!Float.isNaN(motionKeyPosition.mAltPercentX)) {
            f15 = motionKeyPosition.mAltPercentX;
        }
        this.mMode = 0;
    ...
    void initPath(MotionKeyPosition motionKeyPosition, MotionPaths motionPaths, MotionPaths motionPaths2) {
        ...
        float f16 = Float.isNaN(motionKeyPosition.mPercentY) ? BitmapDescriptorFactory.HUE_RED : motionKeyPosition.mPercentY;
        this.mMode = 1;
    ...
    void setView(float f, MotionWidget motionWidget, int[] iArr, double[] dArr, double[] dArr2, double[] dArr3) {
        ...
        if (motion != null) {
            ...
            f2 = f7;
            if (!Float.isNaN(f15)) {
                motionWidget.setRotationZ((float) (BitmapDescriptorFactory.HUE_RED + f15 + Math.toDegrees(Math.atan2(f10 + (f12 / 2.0f), f9 + (f11 / 2.0f)))));
            }
        }
        float f20 = f4 + 0.5f;
        float f21 = f5 + 0.5f;
