...
public class CamColor {
    ...
    public static int toColor(@FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, to = 360.0d) float f, @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, to = DoubleCompanionObject.POSITIVE_INFINITY, toInclusive = false) float f2, @FloatRange(from = AudioStats.AUDIO_AMPLITUDE_NONE, to = 100.0d) float f3) {
    ...
    int viewed(ViewingConditions viewingConditions) {
        float fPow = (float) Math.pow(((((double) getChroma()) == AudioStats.AUDIO_AMPLITUDE_NONE || ((double) getJ()) == AudioStats.AUDIO_AMPLITUDE_NONE) ? BitmapDescriptorFactory.HUE_RED : getChroma() / ((float) Math.sqrt(getJ() / 100.0d))) / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d), 1.1111111111111112d);
