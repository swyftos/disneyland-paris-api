...
public final class WindowMetricsCalculatorCompat implements WindowMetricsCalculator {
    ...
    public WindowMetrics computeCurrentWindowMetrics(@NotNull Activity activity) throws IllegalAccessException, NoSuchFieldException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        if (Build.VERSION.SDK_INT >= 30) {
            rectComputeWindowBoundsQ$window_release = ActivityCompatHelperApi30.INSTANCE.currentWindowBounds(activity);
        } else {
            rectComputeWindowBoundsQ$window_release = computeWindowBoundsQ$window_release(activity);
        }
        return new WindowMetrics(rectComputeWindowBoundsQ$window_release);
    }
