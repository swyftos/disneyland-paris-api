...
public final class SimpleTimeLimiter implements TimeLimiter {
    ...
    private static Object newProxy(Class cls, InvocationHandler invocationHandler) {
    ...
    public Object callWithTimeout(Callable callable, long j, TimeUnit timeUnit, boolean z) throws Exception {
        ...
        Preconditions.checkNotNull(timeUnit);
        checkPositiveTimeout(j);
        Future futureSubmit = this.executor.submit(callable);
        try {
            if (z) {
                futureSubmit = futureSubmit.get(j, timeUnit);
            } else {
                futureSubmit = Uninterruptibles.getUninterruptibly(futureSubmit, j, timeUnit);
            }
            return futureSubmit;
    ...
    public <T> T callWithTimeout(Callable<T> callable, long j, TimeUnit timeUnit) throws ExecutionException, InterruptedException, TimeoutException {
        Preconditions.checkNotNull(callable);
        Preconditions.checkNotNull(timeUnit);
        checkPositiveTimeout(j);
    ...
    @Override // com.google.common.util.concurrent.TimeLimiter
    public void runWithTimeout(Runnable runnable, long j, TimeUnit timeUnit) throws ExecutionException, InterruptedException, TimeoutException {
    ...
    private static Set findInterruptibleMethods(Class cls) throws SecurityException {
