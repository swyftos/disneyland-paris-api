...
final class RopeByteString extends ByteString {
    ...
    private boolean equalsFragments(ByteString byteString) {
        ...
        while (true) {
            ...
            if (i == 0) {
                zEqualsRange = leafByteString.equalsRange(leafByteString2, i2, iMin);
            } else {
                zEqualsRange = leafByteString2.equalsRange(leafByteString, i, iMin);
            }
            if (!zEqualsRange) {
                return false;
            }
    ...
    private static class Balancer {
        ...
        private void insert(ByteString byteString) {
            ...
            ByteString ropeByteString = (ByteString) this.prefixesStack.pop();
            while (true) {
                if (this.prefixesStack.isEmpty() || ((ByteString) this.prefixesStack.peek()).size() >= iMinLength2) {
                    break;
                } else {
                    ropeByteString = new RopeByteString((ByteString) this.prefixesStack.pop(), ropeByteString);
                }
            ...
            while (!this.prefixesStack.isEmpty()) {
                if (((ByteString) this.prefixesStack.peek()).size() >= RopeByteString.minLength(getDepthBinForLength(ropeByteString2.size()) + 1)) {
                    break;
                } else {
