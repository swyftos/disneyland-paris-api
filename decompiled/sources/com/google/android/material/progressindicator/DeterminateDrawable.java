        ...
        if (!getBounds().isEmpty() && isVisible() && canvas.getClipBounds(rect)) {
            ...
            if (iClamp > 0) {
                if (!(this.drawingDelegate instanceof LinearDrawingDelegate)) {
                    iClamp = (int) ((iClamp * MathUtils.clamp(getIndicatorFraction(), BitmapDescriptorFactory.HUE_RED, 0.01f)) / 0.01f);
                }
                this.drawingDelegate.fillTrack(canvas, this.paint, getIndicatorFraction(), 1.0f, this.baseSpec.trackColor, getAlpha(), iClamp);
            } else {
                this.drawingDelegate.fillTrack(canvas, this.paint, BitmapDescriptorFactory.HUE_RED, 1.0f, baseProgressIndicatorSpec.trackColor, getAlpha(), 0);
            }
