...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    public void animateContentOut(int i, int i2) {
        ...
        long j = i2;
        long j2 = i;
        this.messageView.animate().alpha(BitmapDescriptorFactory.HUE_RED).setDuration(j).setInterpolator(this.contentInterpolator).setStartDelay(j2).start();
        if (this.actionView.getVisibility() == 0) {
            this.actionView.setAlpha(1.0f);
            this.actionView.animate().alpha(BitmapDescriptorFactory.HUE_RED).setDuration(j).setInterpolator(this.contentInterpolator).setStartDelay(j2).start();
        }
    }
