...
public class IntentModule extends NativeIntentAndroidSpec {
    ...
    public void sendIntent(@Nullable String action, @Nullable ReadableArray extras, @NotNull Promise promise) {
        ...
        if (action == null || action.length() == 0) {
            promise.reject(new JSApplicationIllegalArgumentException("Invalid Action: " + action + InstructionFileId.DOT));
            return;
        }
        Intent intent = new Intent(action);
        PackageManager packageManager = getReactApplicationContext().getPackageManager();
        if (packageManager == null || intent.resolveActivity(packageManager) == null) {
            promise.reject(new JSApplicationIllegalArgumentException("Could not launch Intent with action " + action + InstructionFileId.DOT));
            return;
        }
        if (extras != null) {
