...
public class DevServerHelper {
    ...
    public DevServerHelper(DeveloperSettings developerSettings, Context context, PackagerConnectionSettings packagerConnectionSettings) {
        ...
        OkHttpClient okHttpClientBuild = builder.connectTimeout(5000L, timeUnit).readTimeout(0L, timeUnit).writeTimeout(0L, timeUnit).build();
    ...
    private String createBundleURL(String str, BundleType bundleType, String str2, boolean z, boolean z2) {
        ...
        sb2.append(String.format(Locale.US, "http://%s/%s.%s?platform=android&dev=%s&lazy=%s&minify=%s&app=%s&modulesOnly=%s&runModule=%s", str2, str, bundleType.typeID(), Boolean.valueOf(devMode), Boolean.valueOf(devMode), Boolean.valueOf(getJSMinifyMode()), this.mPackageName, z ? Characteristics.TRUE : "false", z2 ? Characteristics.TRUE : "false"));
    ...
    public void openDebugger(@Nullable final ReactContext reactContext, final String str) {
        Request.Builder builderMethod = new Request.Builder().url(String.format(Locale.US, "http://%s/open-debugger?device=%s", this.mPackagerConnectionSettings.getDebugServerHost(), Uri.encode(getInspectorDeviceId()))).method("POST", RequestBody.create((MediaType) null, ""));
        OkHttp3.Request.Builder.build.Enter(builderMethod);
        this.mClient.newCall(builderMethod.build()).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.5
