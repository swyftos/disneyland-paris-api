...
class PdfFile {
    ...
    private SizeF maxHeightPageSize = new SizeF(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED);
    private SizeF maxWidthPageSize = new SizeF(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED);
    private List pageOffsets = new ArrayList();
    private List pageSpacing = new ArrayList();
    private float documentLength = BitmapDescriptorFactory.HUE_RED;
    ...
    public void recalculatePageSizes(Size size) {
        ...
        while (it.hasNext()) {
            this.pageSizes.add(pageSizeCalculator.calculate((Size) it.next()));
        }
        if (this.autoSpacing) {
    ...
    private void prepareDocLen() {
        ...
        for (int i = 0; i < getPagesCount(); i++) {
            ...
            if (this.autoSpacing) {
                fFloatValue = ((Float) this.pageSpacing.get(i)).floatValue();
    ...
    private void preparePagesOffset() {
        ...
        for (int i = 0; i < getPagesCount(); i++) {
            ...
            if (this.autoSpacing) {
                fFloatValue2 += ((Float) this.pageSpacing.get(i)).floatValue() / 2.0f;
                if (i == 0) {
                ...
                fFloatValue = ((Float) this.pageSpacing.get(i)).floatValue() / 2.0f;
            } else {
                this.pageOffsets.add(Float.valueOf(fFloatValue2));
                fFloatValue = this.spacingPx;
            }
    ...
    public float getPageSpacing(int i, float f) {
        return (this.autoSpacing ? ((Float) this.pageSpacing.get(i)).floatValue() : this.spacingPx) * f;
    }

    public float getPageOffset(int i, float f) {
        return documentPage(i) < 0 ? BitmapDescriptorFactory.HUE_RED : ((Float) this.pageOffsets.get(i)).floatValue() * f;
    }
    ...
    public int getPageAtOffset(float f, float f2) {
        int i = 0;
        for (int i2 = 0; i2 < getPagesCount() && (((Float) this.pageOffsets.get(i2)).floatValue() * f2) - (getPageSpacing(i2, f2) / 2.0f) < f; i2++) {
            i++;
        }
        int i3 = i - 1;
        if (i3 >= 0) {
    ...
    public List getPageLinks(int i) {
