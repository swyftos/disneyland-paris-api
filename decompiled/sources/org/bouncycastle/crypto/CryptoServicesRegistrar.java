...
public final class CryptoServicesRegistrar {
    ...
    private static void checkPermission(final Permission permission) {
        ...
        if (securityManager != null) {
            AccessController.doPrivileged(new PrivilegedAction() { // from class: org.bouncycastle.crypto.CryptoServicesRegistrar.1
    ...
    public static <T> T[] clearGlobalProperty(Property property) {
        ...
        return (T[]) ((Object[]) globalProperties.remove(property.name));
    }
    ...
    private static void localSetGlobalProperty(Property property, Object... objArr) {
        if (!property.type.isAssignableFrom(objArr[0].getClass())) {
            throw new IllegalArgumentException("Bad property value passed");
        }
        localSetThread(property, objArr);
        globalProperties.put(property.name, objArr);
    }

    private static void localSetThread(Property property, Object[] objArr) {
        ThreadLocal threadLocal = threadProperties;
        Map map = (Map) threadLocal.get();
        if (map == null) {
            map = new HashMap();
