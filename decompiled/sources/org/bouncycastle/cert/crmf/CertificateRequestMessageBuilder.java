    ...
    public CertificateRequestMessage build() throws CRMFException {
        ...
        if (this.popSigner != null) {
            ...
            if (certTemplate.getSubject() == null || certTemplate.getPublicKey() == null) {
                ...
                proofOfPossession2 = new ProofOfPossession(proofOfPossessionSigningKeyBuilder.build(this.popSigner));
            } else {
                proofOfPossession2 = new ProofOfPossession(new ProofOfPossessionSigningKeyBuilder(certRequest).build(this.popSigner));
            }
            aSN1EncodableVector3.add(proofOfPossession2);
        } else {
            POPOPrivKey pOPOPrivKey = this.popoPrivKey;
            if (pOPOPrivKey != null) {
                aSN1EncodableVector3.add(new ProofOfPossession(this.popoType, pOPOPrivKey));
            } else {
                PKMACValue pKMACValue = this.agreeMAC;
                if (pKMACValue != null) {
                    proofOfPossession = new ProofOfPossession(3, POPOPrivKey.getInstance(new DERTaggedObject(false, 3, pKMACValue)));
