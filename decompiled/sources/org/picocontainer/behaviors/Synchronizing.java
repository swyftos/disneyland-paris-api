...
public class Synchronizing extends AbstractBehaviorFactory {
    ...
    public <T> ComponentAdapter<T> createComponentAdapter(ComponentMonitor componentMonitor, LifecycleStrategy lifecycleStrategy, Properties properties, Object obj, Class<T> cls, Parameter... parameterArr) {
        if (AbstractBehaviorFactory.removePropertiesIfPresent(properties, Characteristics.NO_SYNCHRONIZE)) {
            return super.createComponentAdapter(componentMonitor, lifecycleStrategy, properties, obj, cls, parameterArr);
        }
        AbstractBehaviorFactory.removePropertiesIfPresent(properties, Characteristics.SYNCHRONIZE);
        return componentMonitor.newBehavior(new Synchronized(super.createComponentAdapter(componentMonitor, lifecycleStrategy, properties, obj, cls, parameterArr)));
    }
    ...
    public <T> ComponentAdapter<T> addComponentAdapter(ComponentMonitor componentMonitor, LifecycleStrategy lifecycleStrategy, Properties properties, ComponentAdapter<T> componentAdapter) {
        ...
        AbstractBehaviorFactory.removePropertiesIfPresent(properties, Characteristics.SYNCHRONIZE);
        return componentMonitor.newBehavior(new Synchronized(super.addComponentAdapter(componentMonitor, lifecycleStrategy, properties, componentAdapter)));
    }
}
